#!/usr/bin/env python3
"""
Audit all reports against EDUSRC requirements.
Check: HTTP packets present, 200 OK verification, EDUSRC rules compliance.
"""
import os
import re
import json
...
VULN_TYPES = {
    ...
    "北京大学": {
        "严重": ["SQL注入", "RCE", "反序列化", "未授权访问敏感数据", "SSRF(可内网探测)", "越权(批量数据)", "XXE", "SSTI"],
        "高危": ["文件上传/任意文件写入", "命令执行", "存储型XSS", "CSRF(敏感操作)", "越权(单条)", "逻辑漏洞", "路径遍历", "接口鉴权缺失"],
        "中危": ["信息泄露(500-10W)", "CSRF", "SSRF(有限制)", "逻辑缺陷", "弱口令", "反射XSS", "URL跳转"],
        "低危": ["少量信息泄露", "短信轰炸", "Self-XSS", "CORS配置缺陷"],
        "忽略": ["无敏感CSRF", "内网IP泄露", "Swagger不可利用"]
    },
    "浙江旅游职业学院": {
        "严重": ["RCE", "SQL注入", "核心权限", "10W+信息泄露"],
        "高危": ["文件上传", "越权", "存储XSS", "未授权敏感数据", "SSRF"],
        "中危": ["信息泄露", "CSRF", "弱口令", "逻辑缺陷", "SSRF有限制"],
        "低危": ["少量信息泄露", "反射XSS", "组件版本过低"],
        "忽略": ["短信轰炸", "Self-XSS"]
    },
    "智元创新（上海）科技股份有限公司": {
        "严重": ["核心权限", "大量敏感数据泄露", "RCE"],
        "高危": ["信息泄露(1W+)", "越权", "未授权访问", "SSRF", "逻辑漏洞"],
        "中危": ["信息泄露", "前端配置泄露", "CSRF", "弱口令"],
        "低危": ["少量信息泄露", "反射XSS", "URL跳转"],
        "忽略": ["Self-XSS", "短信轰炸", "内网IP泄露"]
    },
    "华住酒店": {
        "高危": ["RCE", "SQL注入", "严重信息泄露", "越权", "逻辑漏洞"],
        "中危": ["信息泄露", "CSRF", "存储XSS", "弱口令"],
        "低危": ["轻微信息泄露", "反射XSS", "短信轰炸"]
    },
    "上海春芝堂生物制品有限公司": {
