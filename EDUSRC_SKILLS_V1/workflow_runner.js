...
function getStructuredCallSites(globalName = '__p2_call_sites_json') {
...
function formatStructuredCallSiteContext(globalName = '__p2_call_sites_json') {
...
p1_assets = await agent(
  ...
  4. **特权资产识别（核心策略 — 优先找"别人挖不了"的目标）**
     - SRC 太多人挖，同质化严重。真正的洞在**特权资产**里：认证/白名单/管理员才能用的核心功能（教育网同理：学生测不到的管理面、教师/管理员权限、付费数据库）。命中任一信号即标 [特权资产]：
     · [认证/白名单] — 统一认证/统一身份/SSO/CAS/IAAA 保护的系统；需学号/工号/管理员才能用
     · [管理/运营/内部] — 管理后台/运营后台/内部系统/控制台/仪表板/运维（管理员权限面 = 别人测不到）
     · [付费/文献] — 图书馆付费数据库/知网万方/文献数据库/期刊（需账号或校园网认证）
     · [教务/人事敏感] — 教务系统/成绩/学籍/人事/财务/一卡通（特权数据面）
     · [新业务/冷门] — 新功能/冷门子站/测试灰度/UAT/特殊端口（别人不测 = 捡漏）
     - 每个 [特权资产] 在 reason 注明"为什么别人挖不到"，category_breakdown 统计 "privileged": N

  5. **优先级排序输出**
     - **最高**: [特权资产]（认证/管理/付费/敏感数据 — 优先测试）
...
${p2_discoveries_text != null && p2_discoveries_text !== '' ? p2_discoveries_text : typeof p2_discoveries_text === 'string' ? '（JS分析了但无发现）' : '（无 JS 分析数据）'}
