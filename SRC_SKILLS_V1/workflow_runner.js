...
p1_assets = await agent(
  ...
  4. **特权资产识别（核心策略 — 优先找"别人挖不了"的目标）**
     SRC 太多人挖，同质化严重。真正的洞在**特权资产**里：企业付费/认证/白名单才能用的核心业务功能。逐个资产判断是否命中特权信号（命中任一即标 [特权资产] 并 priority=最高）：
     · [付费/企业版] — 标题/页面/JS 含"企业版/专业版/旗舰版/付费/收费/会员/VIP/认证企业/专属/私有化/授权/开通"等，或功能需付费/企业认证才能用
     · [认证/白名单] — 开放平台/开放 API/开发者门户 需企业认证或 AppKey；登录后才有完整功能；白名单制接口面
     · [管理/运营/内部] — 管理后台/运营后台/内部系统/控制台/仪表板/运维（管理员权限面 = 别人测不到）
     · [新业务/冷门] — 新上线功能、非主站冷门子站、测试/灰度/UAT 环境、特殊端口（别人不测 = 捡漏）
     · [含敏感数据] — 大数据/数据管理/BI/财务/订单/支付/用户信息/日志类（特权数据面）
     - 对每个 [特权资产] 在 reason 里注明**为什么别人挖不到**（付费墙/认证门槛/冷门/内部/敏感数据），并在 category_breakdown 中统计 "privileged": N
     - 判定口径：宁可多标（后续过滤），不可漏标 — 这是本次挖掘的差异化来源

  5. **优先级排序输出**
     - 最高: [特权资产]（付费/认证/白名单/内部/敏感数据 — 优先测试）
     - 最高: [管理后台][境外资产]
     - 高: [范围内][新发现]
     - 中: [非常见端口][组件指纹]

6. **URL聚合去重**：同域名多端口→保留HTTP+HTTPS各一；同IP多域名→独立保留
...
### 第零级：本质业务原语驱动（最高优先级 — 判定端点"做什么"，而非"叫什么"）

当 Phase 2 已输出 business_attr 时，**按原语直接选攻击基元**（详见上方 BUSINESS_ATTR_GUIDE）；未输出时先从调用链/后端语义自行推断。原语判定优先于一切参数/命名映射。
...
3. SSRF专项测试（参数含 url/path/redirect/domain/host/target 的端点必须逐个测试 SSRF）:
   ...
   **第一性原理推演**（每次绕过尝试前必须执行）:
   ...
   - 后端可能怎么校验输入？(黑名单IP/白名单域名/类型校验/正则匹配)
   - 校验在哪一层？(WAF/API网关/应用层代码) → 层间解析差异即绕过机会
   - 如果我是开发，会怎么防御？在防御盲区寻找突破口
   - 对于IP黑名单: 尝试进制混淆(127.0.0.1=2130706433)、IPv6映射、DNS rebind
   - 对于域名白名单: 尝试子域名可控(@evil.com/evil.com.whitelisted.com)、短链接跳转
...
对每条候选链: 用 curl 只读确认组成端点可达性与鉴权状态，判断各原语能否串成有效危害（如 SSRF端点+登录端点组合）。只记录可串联性判断与组合危害，不做破坏性利用。若组成端点不可达或都需高权限，标记链不可用。
