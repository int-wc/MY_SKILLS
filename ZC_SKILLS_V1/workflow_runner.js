...
if (mode.startsWith('phase5')) {
  ...
  p1_assets = await agent(
    ...
    5. **特权资产识别（核心策略 — 优先找"别人挖不了"的目标）**
       - SRC 太多人挖，同质化严重。真正的洞在**特权资产**里：付费/认证/白名单才能用的核心功能。逐个资产判断是否命中特权信号（命中任一即标 [特权资产]）：
       · [付费/企业版] — 标题/JS 含"企业版/专业版/旗舰版/付费/收费/会员/VIP/认证/专属/私有化/授权/开通"
       · [认证/白名单] — 开放平台/开放 API/开发者门户 需认证或 AppKey；登录后才有完整功能
       · [管理/运营/内部] — 管理后台/运营后台/内部系统/控制台/仪表板/运维
       · [新业务/冷门] — 新功能/冷门子站/测试灰度/UAT/特殊端口（别人不测 = 捡漏）
       · [含敏感数据] — 大数据/BI/财务/订单/支付/用户信息类
  ...
  建议: ${f.notes || '按攻击面逐项测试'}`
    ...
    for (const ft of fuzz_targets_zc) {
      ...
      try {
        ...
        `执行 fuzz(VPN) 并返回【结构化结果】（不要输出散文总结，不要添加前言/分析/结论）。
...
⚠️ 若结果JSON读取失败，findings 返回空数组，不要编造数据。`,
        { label: `🤖 fuzz: ${ft}`, phase: '深度分析', schema: FUZZ_SCHEMA_ZC }
      )
      } catch(e) {
      ...
      try {
        let fuzzData = null
        if (fuzz_raw_zc && typeof fuzz_raw_zc === 'object' && Array.isArray(fuzz_raw_zc.findings)) {
          fuzzData = fuzz_raw_zc   // schema 强制结构化返回
        } else {
          // 兜底：旧文本格式解析
